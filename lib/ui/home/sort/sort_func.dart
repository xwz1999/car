...
class SortFunc {
  ...
  static Future<List<SortSeriesModel>> getSeriesList(int brandId) async {
    var model = await apiClient
        .request(API.home.sort.getCarSeries, data: {'brandId': brandId});
    if ((model.data as List).isEmpty) return [];
    return (model.data as List)
        .map((e) => SortSeriesModel.fromJson(e))
        .toList();
  }
  ...
  static Future<List<SortCarModelModel>> getModelList(int seriesId) async {
    var model = await apiClient
        .request(API.home.sort.getCarModel, data: {'seriesId': seriesId});
    if ((model.data as List).isEmpty) return [];
    return (model.data as List)
        .map((e) => SortCarModelModel.fromJson(e))
        .toList();
  }
}
