...
class ConsignmentOrderWidget extends StatefulWidget {
  final Function callBack;

  const ConsignmentOrderWidget({super.key, required this.callBack});

  @override
  State<ConsignmentOrderWidget> createState() => _ConsignmentOrderWidgetState();
}

class _ConsignmentOrderWidgetState extends State<ConsignmentOrderWidget> {
      ...
      child: Column(
        children: [
          ...
          SizedBox(
            height: 88.w,
            child: CarWidget(
                items: ConsignmentSearchStatus.values.map((e) => e.typeStr)
                    .toList(),
                callBack: (index) {
          ...
          Expanded(
            child: EasyRefresh(
              ...
              onRefresh: () async {
                _page = 1;
                _consignmentList =
                await OrderFunc.getLists(page: _page, size: _size, data:_params);
                _onLoad = false;
                setState(() {});
              },
              onLoad: () async {
